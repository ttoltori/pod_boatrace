...
public class Betting1T extends BettingDefault {
	...
	private WekaPattern convert(String kumiban, List<StatMlPtnFiltered> listPattern, OddsItemEx odds) {
		...
		for (StatMlPtnFiltered ptn : listPattern) {
			// オッズ、オッズランクが範囲外なら除外
			if (odds.value < ptn.getMinOdds() || odds.value > ptn.getMaxOdds()
					|| odds.rank < ptn.getMinOddsrank() || odds.rank > ptn.getMaxOddsrank() ) {
				continue;
			}
