 ...
 * @author ttolt
 */
public class XYRangeFinder2 {
	List<RangeStatUnit> xylist;
	List<RangeStatUnit> xlist;
	List<RangeStatUnit> ylist;
	
	RangeSumUnit bestRangeSumUnit;
	
	public XYRangeFinder2(List<RangeStatUnit> xylist, List<RangeStatUnit> xlist, List<RangeStatUnit> ylist) {
		super();
		this.xylist = xylist;
		this.xlist = xlist;
		this.ylist = ylist;
	...
	void parse() {
		...
		List<RangeSumUnit> listRangeUnit = new ArrayList<>(); 

		for (int x1 = 0; x1 < xSize; x1++) {
			for (int x2 = x1; x2 < xSize; x2++) {
				for (int y1 = 0; y1 < ySize; y1++) {
					for (int y2 = y1; y2 < ySize; y2++) {
						listRangeUnit.add(new RangeSumUnit(x1, x2, y1, y2));
					}
				}
			}
		}

		listRangeUnit.sort(new RangeSumUnitComparator());
		bestRangeSumUnit = listRangeUnit.get(listRangeUnit.size()-1);
	}
	...
	private class RangeSumUnit {
		...
		public RangeSumUnit(int x1, int x2, int y1, int y2) {
			...
			this.y1 = y1;
			this.y2 = y2;
			setRangeUnits(x1, x2, y1, y2);
		}

		/** index x1～x2, y1～y2のxy範囲の集計及び合計収益金額を取得する　*/
		void setRangeUnits(int x1, int x2, int y1, int y2) {
			Double x1Factor = xlist.get(x1).factorValue();
			Double x2Factor = xlist.get(x2).factorValue();
			Double y1Factor = ylist.get(y1).factorValue();
			Double y2Factor = ylist.get(y2).factorValue();
			
			for (RangeStatUnit unit : xylist) {
				Double[] factorValues = unit.factorValues();
				Double beforeOdds = factorValues[0];
				Double probability = factorValues[1];
				
				if (beforeOdds < x1Factor || beforeOdds > x2Factor ) {
					continue;
				}
				
				if (probability < y1Factor || probability > y2Factor ) {
					continue;
				}
