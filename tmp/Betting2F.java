...
public class Betting2F extends BettingDefault {
	...
	private WekaPattern convert(String kumiban, List<StatMlPtnFiltered> listPattern, OddsItemEx odds) {
		...
		for (StatMlPtnFiltered ptn : listPattern) {
			// オッズ、オッズランクが範囲外なら除外
			if (odds.value < ptn.getMinOdds()+2 || odds.value > ptn.getMaxOdds()+2 || odds.rank < ptn.getMinOddsrank()
					|| odds.rank > ptn.getMaxOddsrank()) {
				continue;
			}

//			if (!kumiban.equals("14")) {
//				continue;
//			}
//			if (ptn.getPtnHitrate() < 35) {
//				continue;
//			}
