 ...
 * @author ttolt
 */
public class XYRangeFinderBk {
	List<RangeStatUnit> xlist;
	List<RangeStatUnit> ylist;
	Map<String, RangeStatUnit> mapXYStatUnit;
	RangeSumUnit bestRangeSumUnit = null;
	
	public XYRangeFinderBk(Map<String, RangeStatUnit> mapXYStatUnit, List<RangeStatUnit> xlist, List<RangeStatUnit> ylist) {
		super();
	...
	void parse() {
		...
		RangeSumUnit rangeSumUnit;
		bestRangeSumUnit = new RangeSumUnit(0, 0, 0, 0);
		for (int x1 = 0; x1 < xSize; x1++) {
			for (int x2 = x1; x2 < xSize; x2++) {
				for (int y1 = 0; y1 < ySize; y1++) {
					for (int y2 = y1; y2 < ySize; y2++) {
						rangeSumUnit = new RangeSumUnit(x1, x2, y1, y2);
						if (rangeSumUnit.sum > bestRangeSumUnit.sum) {
							bestRangeSumUnit = rangeSumUnit;
						}
					}
				}
			}
	...
	private class RangeSumUnit {
		...
		public int y2;
		public int sum;
		public List<RangeStatUnit> units;
