-- kumibanl単位生成
DROP FUNCTION IF EXISTS ranking_777_wk(int, int, VARCHAR(40), VARCHAR(250), VARCHAR(200), VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION ranking_777_wk(
pFactorNo int,
pRanking int,
  ...
  BEGIN
    ...
    if pKumiban != null then
	    EXECUTE format ('
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat777_1 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' and patternid like ''%%wk%%'' ) s1
								  , (select keycol from stat777_2 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' and patternid like ''%%wk%%'' ) s2
			...
			order by ranking
		', factor, pFactorValue, pCond, pResultType, pKumiban, pCond, pResultType, pKumiban, pRanking
	    ...
	    EXECUTE format ('
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat777_1 where (%s) and result_type = ''%s'' and bettype = any($1) and patternid like ''%%wk%%'' ) s1
								  , (select keycol from stat777_2 where (%s) and result_type = ''%s'' and bettype = any($1) and patternid like ''%%wk%%'' ) s2
								where s1.keycol = s2.keycol 
							) t123,
							stat777_3 sr
							where t123.keycol = sr.keycol
			...
			where sr.keycol2 = pidr.keycol2
			order by ranking
		', factor, pFactorValue, pCond, pResultType, pCond, pResultType, pRanking
