-- kumibanl単位生成
DROP FUNCTION IF EXISTS ranking_777(int, int, VARCHAR(40), VARCHAR(250), VARCHAR(200), VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION ranking_777(
pFactorNo int,
pRanking int,
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat777_1 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s1
								  , (select keycol from stat777_2 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s2
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat777_1 where (%s) and result_type = ''%s'' and bettype = any($1) ) s1
								  , (select keycol from stat777_2 where (%s) and result_type = ''%s'' and bettype = any($1) ) s2
								where s1.keycol = s2.keycol 
							) t123,
							stat777_3 sr
							where t123.keycol = sr.keycol
