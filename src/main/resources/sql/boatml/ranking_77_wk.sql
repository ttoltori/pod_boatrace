-- kumibanl単位生成
DROP FUNCTION IF EXISTS ranking_77_wk(int, int, VARCHAR(40), VARCHAR(250), VARCHAR(200), VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION ranking_77_wk(
pFactorNo int,
pRanking int,
  ...
  BEGIN
    ...
    if pKumiban != null then
	    EXECUTE format ('
			...
			order by ranking
		', factor, pFactorValue, pCond, pResultType, pKumiban, pRanking
		 , pCond, pResultType, pKumiban
		) USING pBettypeList;
	else
	    EXECUTE format ('
			insert into ptn_ranking
			...
			where sr.keycol2 = pidr.keycol2
			order by ranking
		', factor, pFactorValue, pCond, pResultType, pRanking
		 , pCond, pResultType
		) USING pBettypeList;
	end if;
