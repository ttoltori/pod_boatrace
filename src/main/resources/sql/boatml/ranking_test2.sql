-- kumibanl単位生成
DROP FUNCTION IF EXISTS ranking_test2(int, int, VARCHAR(40), VARCHAR(250), VARCHAR(200), VARCHAR(1), VARCHAR(2)[], VARCHAR(3), VARCHAR(20)[]); 
CREATE OR REPLACE FUNCTION ranking_test2(
pFactorNo int,
pRanking int,
  ...
  BEGIN
    ...
    if pKumiban is not null then
	    EXECUTE format ('
						...
						from (
							...
							(
								...
								from
								  ...
								  , (select keycol from %s where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s3
			...
			order by ranking
		', factor, pFactorValue, pTables[1], pCond, pResultType, pKumiban, pTables[2], pCond, pResultType, pKumiban
		 , pTables[3], pCond, pResultType, pKumiban, pTables[4], pRanking
		 , pTables[1], pCond, pResultType, pKumiban, pTables[2], pCond, pResultType, pKumiban
		 , pTables[3], pCond, pResultType, pKumiban, pTables[4] 
		) USING pBettypeList;
	else
	    EXECUTE format ('
			insert into ptn_ranking
						...
						from (
							...
							(
								...
								from
								  ...
								  , (select keycol from %s where (%s) and result_type = ''%s'' and bettype = any($1) ) s3
								where s1.keycol = s2.keycol and s1.keycol = s3.keycol
							) t123,
							%s sr
							where t123.keycol = sr.keycol
			...
			where sr.keycol2 = pidr.keycol2
			order by ranking
		', factor, pFactorValue, pTables[1], pCond, pResultType, pTables[2], pCond, pResultType
		 , pTables[3], pCond, pResultType, pTables[4], pRanking
		 , pTables[1], pCond, pResultType, pTables[2], pCond, pResultType
		 , pTables[3], pCond, pResultType, pTables[4]
		) USING pBettypeList;
	end if;
