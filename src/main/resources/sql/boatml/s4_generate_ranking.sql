-- kumibanl単位生成
DROP FUNCTION IF EXISTS s4_generate_ranking(VARCHAR(20), VARCHAR(250), VARCHAR(200), int, VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION s4_generate_ranking (
...
RETURNS VOID AS $$
  begin
    ...
    EXECUTE format ('
					...
					from (
						...
						(
							select
							  s1.keycol
							from
							    (select keycol from stat_bork12 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s1
						) t123,
						stat_bork3 sr
		...
		order by ranking
	', pFactorName, pFactorValue, pCond, pResultType, pKumiban, pRanking
