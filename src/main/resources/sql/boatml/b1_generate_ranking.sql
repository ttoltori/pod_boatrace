-- kumibanl単位生成
DROP FUNCTION IF EXISTS b1_generate_ranking(VARCHAR(20), VARCHAR(250), VARCHAR(200), int, VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION b1_generate_ranking(
					...
					from (
						...
						(
							select
							  s1.keycol
							from
							    (select keycol from stat_2018 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s1
							  , (select keycol from stat_2019 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s2
							  , (select keycol from stat_2020 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s3
