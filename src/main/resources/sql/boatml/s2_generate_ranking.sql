-- kumibanl単位生成
DROP FUNCTION IF EXISTS s2_generate_ranking(VARCHAR(20), VARCHAR(250), VARCHAR(200), int, VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION s2_generate_ranking(
...
RETURNS VOID AS $$
  begin
    ...
    EXECUTE format ('
		...
		order by ranking
	', pFactorName, pFactorValue, pCond, pResultType, pKumiban, pCond, pResultType, pKumiban, pRanking
