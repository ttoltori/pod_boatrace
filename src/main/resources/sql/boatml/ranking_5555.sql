-- kumibanl単位生成
DROP FUNCTION IF EXISTS ranking_5555(int, int, VARCHAR(40), VARCHAR(250), VARCHAR(200), VARCHAR(1), VARCHAR(2)[], VARCHAR(3)); 
CREATE OR REPLACE FUNCTION ranking_5555(
pFactorNo int,
pRanking int,
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat5555_1 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s1
								  , (select keycol from stat5555_2 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s2
								  , (select keycol from stat5555_3 where (%s) and result_type = ''%s'' and bettype = any($1) and kumiban = ''%s'' ) s3
						...
						from (
							...
							(
								select
								  s1.keycol
								from
								    (select keycol from stat5555_1 where (%s) and result_type = ''%s'' and bettype = any($1) ) s1
								  , (select keycol from stat5555_2 where (%s) and result_type = ''%s'' and bettype = any($1) ) s2
								  , (select keycol from stat5555_3 where (%s) and result_type = ''%s'' and bettype = any($1) ) s3
								where s1.keycol = s2.keycol and s1.keycol = s3.keycol
							) t123,
							stat5555_4 sr
							where t123.keycol = sr.keycol
