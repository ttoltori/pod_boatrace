...
public class AnalyzeManualPattern extends AnalyzeWekaPatternBase {

	Logger logger = LoggerFactory.getLogger(AnalyzeManualPattern.class);

	public final String TYPE_ALL = "all";
	public final String TYPE_KUMIBAN = "kumiban";
	private String dbNo;
	
	public AnalyzeManualPattern(String propertyFilepath, String dbNo) throws Exception {
	...
	public void execute(KumibanPattern kumibanPattern) throws Exception {
		...
		try (SqlSession session = sqlSessionFactory.openSession(false)) {
			...
			for (FinalizeCondition finalizeCondition : listFinalizeCondition) {
				...
				procedureExecutorMapper.executeFinalizeManualPattern(procParamFinal);
				session.commit();
				finalizeCondCnt++;
	...
	private HashMap<String, Object> createParamFinalizeWekaPattern(KumibanPattern kumibanPattern, FinalizeCondition fcond) {
		...
		// 20190814 manual mode에서는 쓰지 않지만 default값을 설정해준다 
...
//		 "1T",
//		 "1",
//		 "C:/Dev/workspace/Oxygen/pod_boatrace_test/properties/analyze_depth1.properties"
//		 };
		 
		if (args.length != 3 && args.length != 5) {
			System.out.println("args error.");
			System.out.println("ex) AnalyzeManualPattern {db:1} {'all'} {prop file path}");
			System.out.println("ex) AnalyzeManualPattern {db:1} {'kumiban'} {bettype} {kumiban} {propfilepath}");
			System.exit(-1);
		}
		...
		try {
			AnalyzeManualPattern analyze = new AnalyzeManualPattern(propFilepath, dbNo);
			if (type.equals("all")) {
				analyze.execute();
			} else if (type.equals("kumiban")) {
				String betType = args[2];
				String kumiban = args[3];
				analyze.execute(betType, kumiban);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
