...
public class MLGroupSimulationGenerator extends MLResultGenerator {
	/** simulation結果生成クラス */
	...
	public MLGroupSimulationGenerator() {
		super();
	}
	...
	void executeExperiment(String exNo) throws Exception {
        ...
        for (String gradeType : gradeTypeToken) {
            ...
            String evaluationsId = String.join(Delimeter.UNDERBAR.getValue(), 
            		exNo, prop.getString("result_type"), prop.getString("term"),
            		prop.getString("bettype"), prop.getString("kumiban"),
            		prop.getString("groups"),
            		gradeType, prop.getString("probability_type")
            		);
...
//		String exNoList = "1";
		...
		try {
			...
			MLGroupSimulationGenerator generator = new MLGroupSimulationGenerator();
