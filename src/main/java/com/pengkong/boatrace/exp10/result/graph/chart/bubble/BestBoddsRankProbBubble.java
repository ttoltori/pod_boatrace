 ...
 *
 */
public class BestBoddsRankProbBubble extends AbstractDoubleRangeBubbleChart {
	Logger logger = LoggerFactory.getLogger(BestBoddsRankProbBubble.class);

	public BestBoddsRankProbBubble(ResultStat stat, XYRange xyRange) {
		super(stat);
	...
	protected String createTitleLabel() {
		if (mapUnit.size() <= 0) {
			...
			return "BEST直前オッズRANK/確率なし";
		}

		String label;
		label = "Best Boddsrk/Probability BEST(" + dto.getBorkBestmin() + "～" + dto.getBorkBestmax() + " / " + dto.getPrBestmin() + "～"
				+ dto.getPrBestmax() + ")  ";
