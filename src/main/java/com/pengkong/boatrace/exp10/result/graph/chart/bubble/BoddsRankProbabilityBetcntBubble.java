 ...
 *
 */
public class BoddsRankProbabilityBetcntBubble extends AbstractDoubleRangeBubbleChart {
	Logger logger = LoggerFactory.getLogger(BoddsRankProbabilityBetcntBubble.class);

	public BoddsRankProbabilityBetcntBubble(ResultStat stat) {
		super(stat);
	...
	protected String createTitleLabel() {
		if (mapUnit.size() <= 0) {
			logger.warn("no beforeodds rank / probability betcnt data." + stat);
			return "直前オッズRANK/予想確率統計なし";
		}

		String label;
		label = "Boddsrk/Prob betcnt(" + dto.getBorkBestmin() + "～" + dto.getBorkBestmax() + " / " + dto.getPrBestmin() + "～"
				+ dto.getPrBestmax() + ")  ";
