 ...
 *
 */
public class RemoteExpectedClassifier {
	MLPropertyUtil prop = MLPropertyUtil.getInstance();

	Logger logger = LoggerFactory.getLogger(RemoteExpectedClassifier.class);
	...
	public RemoteExpectedClassifier() throws Exception {
	...
	List<Classification> createResult(RemoteRequestSet rrs) throws Exception {
		...
		for (int i = 0; i < rrs.listRes.size(); i++) {
			...
			result.addAll(createClassification(req, res));
		}
	 ...
	 * @throws Exception
	 */
	List<Classification> createClassification(RemoteRequest req, RemoteResponse res) throws Exception {
		...
		for (int i = 0; i < res.values.length; i++) {
			...
			if (res.algorithmId.startsWith("cf_")) { // classification
				...
				RemoteRequestParam param = (RemoteRequestParam) req.param;
				ModelInfo mi = mim.get(param.exNo, param.rankNo, param.pattern, param.ymd);
				clsn.prediction = mi.classValues[i];
				clsn.probability = MathUtil.scale3(res.values[i]);
