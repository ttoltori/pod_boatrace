 ...
 *
 */
public class EvaluationSimulLoaderZEN extends AbstractEvaluationLoader {
	/** sql template */
	...
	void loadDB() throws Exception {
		...
		try {
			...
			sql = sql.replace("{term_1}", prop.getString("term_1"));
			sql = sql.replace("{term_2}", prop.getString("term_2"));
			sql = sql.replace("{term_3}", prop.getString("term_3"));
			...
			if (prop.getString("kumiban").contains("*")) {
				sql = StringUtil.replaceBetween(sql, "<optional_1>", "</optional_1>", true, true, "");
			} else {
				sql = sql.replace("<optional_1>", "");
				sql = sql.replace("</optional_1>", "");
			}

			
			sql = sql.replace("{simul_patternid}", prop.getString("simul_patternid"));
			if (prop.getString("simul_patternid").equals("x")) {
				sql = StringUtil.replaceBetween(sql, "<optional_2>", "</optional_2>", true, true, "");
			} else {
				sql = sql.replace("<optional_2>", "");
				sql = sql.replace("</optional_2>", "");
			}
			
			sql = sql.replaceAll("\\{factor\\}", prop.getString("factor"));
			sql = sql.replaceAll("\\{factor_min\\}", prop.getString("factor_min"));
			sql = sql.replaceAll("\\{factor_max\\}", prop.getString("factor_max"));
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Evaluation> evals = new EvaluationSimulLoaderZEN().getEvaluations();
