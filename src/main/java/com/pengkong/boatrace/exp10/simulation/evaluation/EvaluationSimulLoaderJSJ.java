 ...
 *
 */
public class EvaluationSimulLoaderJSJ extends AbstractEvaluationLoader {
	/** sql template */
	...
	void loadDB() throws Exception {
		...
		try {
			...
			// "3T,3R,3P" -> "'3T','3R','3P'"
			String bettypeList = StringUtil.quote(prop.getString("bettype").split(Delimeter.COMMA.getValue()));
			sql = sql.replace("{bettype_list}", bettypeList);
			...
			sql = sql.replaceAll("\\{factor\\}", prop.getString("factor_" + prop.getString("factor")));
			sql = sql.replaceAll("\\{factor_min\\}", prop.getString("factor_min"));
			
			
			logger.debug("EvaluationSimulLoaderJSJ.sql = " + sql);
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Evaluation> evals = new EvaluationSimulLoaderJSJ().getEvaluations();
