 ...
 *
 */
public class ProbabilityCaculator1234 extends AbstractProbabilityCalculator {
	@Override
	...
	Double getProbability3R(DBRecord rec) {
		return (rec.getDouble("probability1") * rec.getDouble("probability2")) 
				+  (rec.getDouble("probability1") * rec.getDouble("probability3"));  
	}
	...
	Double getProbability3D(DBRecord rec) {
		return ( rec.getDouble("probability1") + rec.getDouble("probability2") ) 
				*  ( rec.getDouble("probability3") * rec.getDouble("probability4") );
	}
	...
	Double getProbability3E(DBRecord rec) {
		return ( rec.getDouble("probability1") * rec.getDouble("probability2") )
				+ ( rec.getDouble("probability1") * rec.getDouble("probability3") )
				+ ( rec.getDouble("probability1") * rec.getDouble("probability4") )
				;
	}
}
