 ...
 *
 */
public class EvaluationSimulLoaderBIG extends AbstractEvaluationLoader {
	/** sql template */
	...
	void loadDB() throws Exception {
		...
		try {
			...
			sql = sql.replace("{models}", StringUtil.quote(prop.getString("models").split(Delimeter.COMMA.getValue())));
			
			sql = sql.replace("{custom}", prop.getString("custom"));

			sql = sql.replace("{table_no}", prop.getString("table_no"));
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Evaluation> evals = new EvaluationSimulLoaderBIG().getEvaluations();
