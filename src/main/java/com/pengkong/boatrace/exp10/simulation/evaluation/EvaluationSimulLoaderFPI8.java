 ...
 *
 */
public class EvaluationSimulLoaderFPI8 extends AbstractEvaluationLoader {
	/** sql template */
	...
	void loadDB() throws Exception {
		...
		try {
			...
			String[] modelToken =  prop.getString("models").split(Delimeter.COMMA.getValue());
			sql = sql.replace("{models}", StringUtil.quote(modelToken));
			
			String[] patternIdToken =  prop.getString("patternids").split(Delimeter.COMMA.getValue());
			sql = sql.replace("{patternids}", StringUtil.quote(patternIdToken));
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Evaluation> evals = new EvaluationSimulLoaderFPI8().getEvaluations();
