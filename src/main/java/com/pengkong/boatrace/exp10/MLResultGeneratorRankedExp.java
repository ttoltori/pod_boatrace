...
import com.pengkong.boatrace.exp10.result.AbstractResultCreatorRankedExp;
 ...
 *
 */
public class MLResultGeneratorRankedExp {
	...
	/** result生成クラス */
	AbstractResultCreatorRankedExp resultCreator;
	
	String configProperty;
	
	public MLResultGeneratorRankedExp()  {
	}
	...
	void setupResultCreator() throws Exception {
		...
		resultCreator = (AbstractResultCreatorRankedExp) c.newInstance();
	}
...
//			MlPrEvaluationMapper mapperPrEval = session.getMapper(MlPrEvaluationMapper.class);
			...
			while (currDate.compareTo(toDate) <= 0) {
				...
				for (DBRecord dbRec : listRec) {
					listResult = createResult(dbRec, resultCreator);
				...
				logger.debug(exNo + " processed..." + listResult.size() + " " + ymd + "/" + resultEndYmd);
				//session.commit();
			...
			if (isSaveStat && isDeleteSavedStat) {
				...
				exam.createCriteria().andEvaluationsIdEqualTo(prop.getString("evaluations_id")).andResultnoEqualTo(exNo).andResultTypeEqualTo(prop.getString("result_type"));
				mapperEval.deleteByExample(exam);
				
				MlRangeEvaluationExample rangeExam = new MlRangeEvaluationExample();
				rangeExam.createCriteria().andEvaluationsIdEqualTo(prop.getString("evaluations_id")).andResultnoEqualTo(exNo).andResultTypeEqualTo(prop.getString("result_type"));
...
//						mapperPrEval.insert(prRec);
	...
	List<MlResult> createResult(DBRecord rec, AbstractResultCreatorRankedExp resultCreator) throws Exception {
	...
	*/	
	...
	public static void main(String[] args) {
		...
		try {
			...
			MLResultGeneratorRankedExp generator = new MLResultGeneratorRankedExp();
