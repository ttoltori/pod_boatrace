...
import com.pengkong.boatrace.service.worker.HeartBeat;
...
public class TohyoManager {
	...
	public void initialize(BettingRule rule) throws Exception {
		this.rule = rule;
	...
	public List<OddsJson> getOddsList(RaceEx race, String type) throws Exception {
		...
		JsonObject jo = gson.fromJson(resJson, JsonObject.class);
	...
	public synchronized int depositIfNeeded() throws Exception {
		...
		if (balance.purchasableBetAmount <= 0) {
			int totalStartBalance = rule.getInt("totalStartBalance");
			// 入金要求
			deposit(totalStartBalance);
			
			Thread.sleep(rule.getInt("waitAfterDeposit"));
	...
	public static void main(String[] args) {
		try {
			...
			tohyoManager.initialize( new BettingRule("C:/Dev/workspace/Oxygen/pod_boatrace/release/properties/usertest.properties"));
