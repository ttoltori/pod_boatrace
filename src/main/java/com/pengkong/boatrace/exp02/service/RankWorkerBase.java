...
public abstract class RankWorkerBase {
	private String description;
	
	private Logger logger = LoggerFactory.getLogger(RankWorkerBase.class);
	...
	protected RankUser user;
	
	public RankWorkerBase(RankUser user, BettingRule rule) throws Exception {
		super();
		this.user = user;
		this.rule = rule;
...
//						race = new RaceProgramPageParser().parseAndSetInfoEx(race, url);
	...
	protected void writeUserStatus(String ymd) throws Exception {
		...
		String filepath = PropertyUtil.getInstance().getString("DIRECTORY_RESULT")  + "user_" + todayYmd + "_"+ rule.getString("type") + ".csv";
	...
	protected void writeRaceInformation(RaceEx race) throws Exception {
		...
		if (sbRaceResult.length() > 0) {
			String filepath = PropertyUtil.getInstance().getString("DIRECTORY_RESULT") + "race_" + todayYmd + "_" + rule.getString("type") + ".csv";
