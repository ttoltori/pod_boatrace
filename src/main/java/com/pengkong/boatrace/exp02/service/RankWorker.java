...
public class RankWorker extends RankWorkerBase implements Runnable {
	private Logger logger = LoggerFactory.getLogger(RankWorker.class);
	
	int bettingSortTime;
	Betting betting = null;
	
	public RankWorker(RankUser user, BettingRule rule) throws Exception{
		super(user, rule);
		initialize(rule);
	}
